...
i. A lot of results/graphs could not be replicated and hence, we are not convinced that Random Forests was necessarily the best model possible.
ii. The results reported strangely did not use out-of-sample data.
iii. Different models were trained on different features. Yet, the author presented a comparison between them without accounting for this discrepancy in features.
iv. The original code given by the author did not contain replication for some important parts such as F1 scores.
v. Also, the two datasets used by the author in the paper to show different countries in which civil war was reported. We don't know why this is, but we expected that similar countries should show civil war cases in both datasets.
