...
NOTE: Other datasets and the ```HS_imputed``` dataset without the split mentioned above will also be utilized in parts where we try to replicate results from the paper. As mentioned, although we don't think in-sample measures are the correct way to assess the performance of any model, we will do so to see if we can **completely** replicate graphs and repoted measures in the original paper.
...
num_folds <- 10
# This line is different than original, but proven to not change anything numerically
...
REP_model_AS_RF <- train(author_spec,
                     ...
                     proximity=F, ntree=3000, ## number of trees grown
                     trControl=tc, data=data)
...
All of the models use the cross-validation training control, ```tc```, implemented in the above code block.
...
Again, note that the author's replicated models are trained on the entire ```HS_imputed``` dataset, and predicted/tested on the entire dataset as shown below.
