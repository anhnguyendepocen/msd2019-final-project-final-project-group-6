...
separationplot(REP_AS_RF_pred_prob$war,
               ...
               height=1.5, col0="white", col1="black", newplot=F)
```
Although we have argued that models are not replicable, the seperation plots we generated for author's replicated models and the ones presented in the original paper are somewhat similar for all of the three logistic regression models. The Random Forests model's seperation plot differs, and this is expected, as this is the model has the highest randomness with both downsampling and training control.

## Replicating F1 scores
```{r f1-score-replication-and-confusion-matrix}
F1_Score(data_test$warstds, REP_FL_uncorrected_pred_raw, positive = "war")
F1_Score(data_test$warstds, REP_FL_penalized_pred_raw, positive = "war")
F1_Score(data_test$warstds, REP_CH_uncorrected_pred_raw, positive = "war")
F1_Score(data_test$warstds, REP_CH_penalized_pred_raw, positive = "war")
F1_Score(data_test$warstds, REP_HS_uncorrected_pred_raw, positive = "war")
F1_Score(data_test$warstds, REP_HS_penalized_pred_raw, positive = "war")
F1_Score(data_test$warstds, REP_AS_RF_pred_raw, positive = "war")
...
2) Boosted Classification Trees: After running some parameter grid searches, we noticed that the Boosted Classification Trees model with the parameters below can achieve an AUC score of 0.934. The Boosted Classification Trees uses Adaptive Boosting which aims to combine a number of weak classifiers and predict based on a weighed voting procedure among these classifiers. More clearly, first, Adaptive Boosting trains a variety of waeak classifiers. Those that have >50% accuracy receive positive weights whereas those that have <50% accuracy receive negative weights. The further the accuracy is from %50, more positive/more negative the weight of that weak classifier. Thus, even those classifiers that are unsuccessful at the task, in other words missclassify most of the time, can be incorporated and used as part of the ensemble.

Unlike Random Forests which is a bagging ensemble model, Boosted Classification Trees is a boosing model. While bagging models aim to decrease variance, boosing models aim to decrese bias. Random Forest trains n Decision Trees on different random subsets of the dataset. Bagging ensemble models train their weak classifiers in parallel. Boosting on the other hand trains the weaker models in series. This sequential training method allows the model to determine which datapoints are misclassified and consequently assign/update the weights of datapoints such that on the next iteration, misclassified datapoints get high probability for classification. Thus, it iteratively corrects the mistakes of its classifiers. Moreover, it is not prone to overfitting.
...
```{r smoted-variation-models-test}
...
F1_Score(data_test$warstds, FL_uncorrected_pred_raw, positive = "war")
...
F1_Score(data_test$warstds, CH_uncorrected_pred_raw, positive = "war")
...
F1_Score(data_test$warstds, HS_uncorrected_pred_raw, positive = "war")
...
F1_Score(data_test$warstds, AS_DT_pred_raw, positive = "war")
F1_Score(data_test$warstds, AS_Boost_pred_raw, positive = "war")
```

```{r Smoted-dataset-confusion-matrix-and-f1-scores}
...
F1_Score(data_test$warstds, AS_uncorrected_smoted_pred_raw, positive = "war")
F1_Score(data_test$warstds, AS_uncorrected_smoted_pred_raw_2, positive = "war")
F1_Score(data_test$warstds, AS_uncorrected_smoted_pred_raw_3, positive = "war")

F1_Score(data_test$warstds, AS_penalized_smoted_pred_raw, positive = "war")
F1_Score(data_test$warstds, AS_penalized_smoted_pred_raw_2, positive = "war")
F1_Score(data_test$warstds, AS_penalized_smoted_pred_raw_3, positive = "war")


F1_Score(data_test$warstds, AS_RF_smoted_pred_raw, positive = "war")
F1_Score(data_test$warstds, AS_RF_smoted_pred_raw_2, positive = "war")
F1_Score(data_test$warstds, AS_RF_smoted_pred_raw_3, positive = "war")
```
...
iii)The seperation plots we generated for author's replicated models and the ones presented in the original paper are somewhat similar for all of the three logistic regression models. However, the plot for Random Forests model was different.
iv) F1 score note - #add here what happens
v) ROC curve

# Analysis for the paper
We observed some glaring issues in the paper -
1. A lot of results/graphs could not be replicated and hence, we are not convinced that Random Forests was necessarily the best model possible.
2. The results reported strangely did not use out-of-sample data.
3. Different models were trained on different features. Yet, the author presented a comparison between them without accounting for this discrepancy in features.
4. The original code given by the author did not contain replication for some important parts such as F1 scores.
